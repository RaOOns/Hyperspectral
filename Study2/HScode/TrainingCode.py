...
def HS_model (data, MODEL, lag_term = 3, comp = 8, SEED = 999, before_y = "yes", hs = "yes", target = "chlorophyll", deep_learning= "NO" ,epochs = 40, batch = 256, Ir = 0.0003) :
    ...
    X_train_series = new_train_X.values.reshape((new_train_X.shape[0], new_train_X.shape[1], 1))
    adam= optimizers.Adam(Ir)
    ...
    if deep_learning == "yes":
        ...
        model_cnn.compile(optimizer = "rmsprop", loss = "mse")
#         model_cnn.summary()
        
        deep_model = model_cnn.fit(X_train_series, np.ravel(train_y), epochs=epochs, verbose=0)
        
        return {"train pls" : train_pls, "train std other" : train_std_other, "train std reflec" : train_std_reflec, "train columns" : train_columns,
                "deep_model" : deep_model, "model_cnn" : model_cnn, "new_train_X" : new_train_X, "new_y" : np.ravel(train_y)}
